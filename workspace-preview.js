...
function injectStyle(ws){
  ...
  style.textContent=`
    ...
    .nsn-brand{display:flex;align-items:center;gap:14px;min-width:0}.nsn-brand img{width:min(360px,45vw);height:auto;display:block;object-fit:contain;object-position:left center}.nsn-powered{font-size:10px;font-weight:800;letter-spacing:.09em;white-space:nowrap}
    ...
    .nsn-hero{background:#fff;border:1px solid #dbe2e8;border-radius:26px;padding:28px;min-height:300px;display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,440px);gap:34px;align-items:center;box-shadow:0 12px 30px rgba(0,22,59,.08)}
    .nsn-hero-copy small{font-weight:900;letter-spacing:.13em}.nsn-hero h1{font-size:clamp(38px,7vw,68px);line-height:.96;letter-spacing:-.045em;margin:12px 0;color:var(--nsn-primary)}.nsn-hero p{max-width:650px;color:#506174;font-size:16px;line-height:1.55}.nsn-hero-logo{width:100%;height:auto;display:block;object-fit:contain}
    ...
    @media(max-width:800px){.nsn-hero{grid-template-columns:1fr}.nsn-hero-logo{width:min(520px,100%);margin:8px auto 0}.nsn-grid.four,.nsn-grid.two{grid-template-columns:1fr 1fr}.nsn-brand img{width:min(300px,58vw)}}
    @media(max-width:520px){.nsn-grid.four,.nsn-grid.two,.nsn-actions{grid-template-columns:1fr}.nsn-main{padding-left:10px;padding-right:10px}.nsn-brand img{width:min(245px,68vw);height:auto}.nsn-powered{display:none}.nsn-nav button{font-size:8px;padding:7px 1px}.nsn-nav button b{font-size:16px}.nsn-hero{padding:22px}.nsn-hero h1{font-size:42px}.nsn-hero-logo{width:100%;margin:12px auto 0}}
  `;
...
function renderPreview(ws){
  ...
  const nav=[['home','⌂','Home'],['activities','◎',t.challenges||'Challenges'],['progress','↗','Progress'],['reports','▥','Reports'],['contribute','★','Contribute'],['lead','⚙',t.rep||'Group Lead']];
  const draw=()=>{
