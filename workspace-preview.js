...
function previewData(){
  return {
    ...
    groupScore:680,
    activities:[
      {title:'Mentoring Connections',description:'Illustrative activity showing how an NSN group could track mentoring engagement.',actual:18,target:50,unit:'sessions',mine:5},
      {title:'Outreach Engagement',description:'Illustrative activity showing how participation and outreach could be made visible.',actual:185,target:500,unit:'people reached',mine:35},
      {title:'Professional Development',description:'Illustrative activity for network learning and development events.',actual:14,target:40,unit:'sessions',mine:3}
    ],
    people:[
...
function screenHome(ws,d){
  return `<section class="nsn-hero"><div class="nsn-hero-copy"><small>POWERED BY KUDOS</small><h1>Recognise. Connect.<br>Contribute.</h1><p>A shared space to recognise contribution, support network activities and make participation visible across the Naval Servicewomen's Network.</p><span class="nsn-strap">CONNECT • CONTRIBUTE • RECOGNISE</span></div><img class="nsn-hero-mark" src="nsn-mark.svg?v=1" alt="NSN mark"></section>
  <div class="nsn-section"><h2>${esc(d.group)} overview</h2><p>Illustrative NSN workspace data</p></div>
  <div class="nsn-grid four"><div class="nsn-card"><div class="nsn-label">My KUDOS score</div><div class="nsn-value">${d.score}</div><div class="nsn-sub">Recognition, ideas and activity contribution</div></div><div class="nsn-card"><div class="nsn-label">Group KUDOS score</div><div class="nsn-value">${d.groupScore}</div><div class="nsn-sub">Combined member contribution</div></div><div class="nsn-card"><div class="nsn-label">Activity completion</div><div class="nsn-value">36%</div><div class="nsn-sub">Average progress across activities</div></div><div class="nsn-card"><div class="nsn-label">Active activities</div><div class="nsn-value">${d.activities.length}</div><div class="nsn-sub">Current network activity portfolio</div></div></div>
  <div class="nsn-section"><h2>Make a contribution</h2><p>Recognition • Ideas</p></div>
  <div class="nsn-actions"><div class="nsn-card nsn-action"><div class="nsn-action-icon">★</div><h3>Recognise someone</h3><p class="nsn-sub">Recognise positive contribution to the network.</p><button class="nsn-btn" data-preview-action>Submit recognition</button></div><div class="nsn-card nsn-action"><div class="nsn-action-icon">↗</div><h3>Share an idea</h3><p class="nsn-sub">Suggest something that could strengthen the network, its activities or member experience.</p><button class="nsn-btn" data-preview-action>Submit idea</button></div></div>
  <div class="nsn-section"><h2>Current activities</h2><p>${d.activities.length} active</p></div><div class="nsn-grid two">${d.activities.map(cardActivity).join('')}</div>`;
}

function screenActivities(ws,d){
  return `<div class="nsn-section"><h2>Group activities</h2><p>Current activities and measurable network goals</p></div><div class="nsn-card"><div class="nsn-field"><label>Group</label><select disabled><option>${esc(d.group)}</option></select></div></div><div class="nsn-section"><h2>${esc(d.group)}</h2><p>${d.activities.length} active activities</p></div><div class="nsn-grid two">${d.activities.map(cardActivity).join('')}</div>`;
}

function screenProgress(ws,d){
  return `<div class="nsn-section"><h2>Progress</h2><p>Group targets and individual contribution</p></div><div class="nsn-card"><div class="nsn-grid two"><div class="nsn-field"><label>Group</label><select disabled><option>${esc(d.group)}</option></select></div><div class="nsn-field"><label>Person</label><select disabled><option>${esc(d.member)}</option></select></div></div></div><div class="nsn-section"><h2>All activities</h2><p>Comparable by percentage complete</p></div><div class="nsn-grid two">${d.activities.map(a=>`<div class="nsn-card"><h3>${esc(a.title)}</h3><div class="nsn-progress"><span style="width:${Math.min(a.actual/a.target,1)*100}%"></span></div><div class="nsn-line"><span>Group: <strong>${fmt(a.actual)} / ${fmt(a.target)} ${esc(a.unit)}</strong></span><span>Selected person: <strong>${fmt(a.mine)}</strong></span></div></div>`).join('')}</div>`;
}
...
function screenReports(ws,d){
  ...
  return `<div class="nsn-section"><h2>Reports</h2><p>Participation, recognition, ideas and activity delivery</p></div><div class="nsn-grid two"><div class="nsn-card"><h3>Network reporting</h3><p class="nsn-sub">NSN reporting can focus on participation and impact without exposing CHF-specific Flight Safety or Performance Shaping Factor measures.</p></div><div class="nsn-card"><h3>Current activity portfolio</h3><div class="nsn-value">${d.activities.length}</div><div class="nsn-sub">Illustrative active activities</div></div></div><div class="nsn-section"><h2>Top KUDOS contributions</h2><p>Illustrative preview</p></div><div class="nsn-table"><table><thead><tr><th>Rank</th><th>Member</th><th>KUDOS</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}

function screenContribute(ws,d){
  return `<div class="nsn-section"><h2>Contribute</h2><p>Recognition and ideas</p></div><div class="nsn-actions"><div class="nsn-card nsn-action"><div class="nsn-action-icon">★</div><h3>Recognition</h3><p class="nsn-sub">Recognise someone for their contribution to the network, its members or its activities.</p><button class="nsn-btn" data-preview-action>Submit recognition</button></div><div class="nsn-card nsn-action"><div class="nsn-action-icon">↗</div><h3>Ideas</h3><p class="nsn-sub">Categories can include Mentoring, Networking, Outreach, Events, Professional Development, Communications and Service Improvement.</p><button class="nsn-btn" data-preview-action>Submit idea</button></div></div><div class="nsn-preview-banner" style="margin-top:16px">Flight Safety and CHF Performance Shaping Factors are disabled for the NSN workspace by configuration.</div>`;
}

function screenLead(ws,d){
  return `<div class="nsn-section"><h2>Group Lead / NSN Admin</h2><p>Scoped workspace administration</p></div><div class="nsn-grid two"><div class="nsn-card"><h3>Group Lead</h3><p class="nsn-sub">Manage your group, moderate group entries, create permitted activities and view group reporting.</p><button class="nsn-btn secondary" data-preview-action>Preview controls</button></div><div class="nsn-card"><h3>NSN Admin</h3><p class="nsn-sub">Manage groups, Group Leads, activity definitions, contribution categories, workspace branding and network-wide reporting.</p><button class="nsn-btn secondary" data-preview-action>Preview controls</button></div></div>`;
}
